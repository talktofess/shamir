...
std::array<std::uint8_t, 32> hash(const std::uint8_t* data, std::size_t len) {
    ...
    // Padded message: data || 0x80 || 0x00... || 64-bit big-endian bit length.
    ...
    for (std::size_t off = 0; off < msg.size(); off += 64) {
        std::uint32_t w[64];
        for (int i = 0; i < 16; ++i) {
            w[i] = (std::uint32_t(msg[off + i * 4]) << 24)
                 | (std::uint32_t(msg[off + i * 4 + 1]) << 16)
                 | (std::uint32_t(msg[off + i * 4 + 2]) << 8)
                 |  std::uint32_t(msg[off + i * 4 + 3]);
        }
        for (int i = 16; i < 64; ++i) {
